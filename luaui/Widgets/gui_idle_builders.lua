		...
		if not buildQueue[1] then
			...
			if not spGetUnitIsBeingBuilt(unitID) then
				...
				if isFactory[udef] then
					...
					if
						(spGetCommandQueue(unitID, 0) == 0)
						and not (spGetUnitMoveTypeData(unitID).aircraftState == "crashing")
					then
						return true
					end
				end
...
local function drawIcon(unitDefID, rect, lightness, zoom, highlightOpacity)
	...
	UiUnit(
		...
		zoom,
		nil,
		math.max(0.1, highlightOpacity or 0.1),
		"#" .. unitDefID,
		nil,
		nil,
		nil,
					...
					if existingGroups[group] then
						...
						if showStack then
							if unitCount > 4 then
								...
								offset = floor((paddedGroupSize - iconSize) / 4)
							elseif unitCount > 3 then
								iconSize = floor(iconSize * 0.83)
								offset = floor((paddedGroupSize - iconSize) / 3)
							elseif unitCount > 2 then
								iconSize = floor(iconSize * 0.86)
								offset = floor((paddedGroupSize - iconSize) / 2)
							elseif unitCount > 1 then
								iconSize = floor(iconSize * 0.88)
								offset = paddedGroupSize - (iconSize * 1.06)
							else
								iconSize = floor(iconSize * 0.94)
								offset = paddedGroupSize - iconSize
							end
						end
...
function widget:ViewResize()
	...
	if WG["unitgroups"] then
		local _, py, sx = WG["unitgroups"].getPosition()
		posY = py / vsy
		posX = (sx + widgetSpaceMargin) / vsx
	end
end
